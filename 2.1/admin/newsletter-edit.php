<?php
/**
 * ADMIN/NEWSLETTER-EDIT.PHP - Edycja kampanii (czyta bloki z bazy)
 */
...
// Pobierz kampanię i bloki z bazy
$campaign = null;
$existingBlocks = [];
...
try {
    ...
    if (!$campaign || $campaign['status'] !== 'draft') {
        $_SESSION['error_message'] = 'Nie można edytować tej kampanii';
        header('Location: /admin/newsletter.php');
        exit;
    }
    
    // Pobierz bloki z tabeli newsletter_blocks
    $stmt = $pdo->prepare("
        SELECT id, block_type, block_order, content 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save') {
    ...
    if (empty($errors)) {
        try {
            ...
            // 1. Aktualizuj kampanię
            $stmt = $pdo->prepare("
                UPDATE newsletter_campaigns 
                SET name = ?, subject = ?, updated_at = NOW()
                WHERE id = ? AND status = 'draft'
            ");
            $stmt->execute([$name, $subject, $campaignId]);
            
            // 2. Usuń stare bloki
            $stmt = $pdo->prepare("DELETE FROM newsletter_blocks WHERE campaign_id = ?");
            $stmt->execute([$campaignId]);
            ...
            $_SESSION['success_message'] = 'Kampania została zaktualizowana';
            header('Location: /admin/newsletter.php');
            exit;
...
<div class="admin-content">
    <div class="page-header">
        <h1><i class="bi bi-pencil"></i> Edycja kampanii</h1>
    ...
    <div class="top-bar">
        <div class="top-bar-grid">
            <div class="form-field">
                ...
                <input type="text" id="campaign-name" value="<?php echo htmlspecialchars($campaign['name']); ?>" placeholder="np. Newsletter Styczeń 2026" required>
            </div>
            <div class="form-field">
                <label>Temat wiadomości <span style="color: #DC2626;">*</span></label>
                <input type="text" id="campaign-subject" value="<?php echo htmlspecialchars($campaign['subject']); ?>" placeholder="np. Promocja -30%" required>
            </div>
        </div>
    </div>
...
<script>
// ZAŁADUJ BLOKI Z BAZY
let blocks = <?php echo json_encode($existingBlocks); ?>;
let selectedBlock = null;
let blockIdCounter = blocks.length > 0 ? Math.max(...blocks.map(b => b.id || 0)) + 1 : 1;

console.log('EDIT MODE: Załadowano ' + blocks.length + ' bloków z bazy danych');
console.log('Bloki:', blocks);
    ...
    const forms = {
        'heading': `
            <div class="settings-group">
                ...
                <div class="help-text">💡 Użyj zmiennych: {{first_name}}</div>
            </div>
            <div class="settings-group">
                <label>Rozmiar</label>
...
// INIT - Załaduj i renderuj bloki z bazy
