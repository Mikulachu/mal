<?php
/**
 * ADMIN/NEWSLETTER.PHP - Panel Newsletter (NAPRAWIONY)
 */
...
if (isset($_GET['duplicate']) && is_numeric($_GET['duplicate'])) {
    try {
        ...
        if ($campaign) {
            $stmt = $pdo->prepare("INSERT INTO newsletter_campaigns (name, subject, status, created_at) VALUES (?, ?, 'draft', NOW())");
            $stmt->execute([
                $campaign['name'] . ' (kopia)',
                $campaign['subject']
            ]);
            ...
            // Kopiuj bloki
            $stmt = $pdo->prepare("SELECT * FROM newsletter_blocks WHERE campaign_id = ?");
            $stmt->execute([$_GET['duplicate']]);
            $blocks = $stmt->fetchAll(PDO::FETCH_ASSOC);
            
            foreach ($blocks as $block) {
                $stmt = $pdo->prepare("INSERT INTO newsletter_blocks (campaign_id, block_type, content, block_order) VALUES (?, ?, ?, ?)");
                $stmt->execute([
                    $newCampaignId,
                    $block['block_type'],
                    $block['content'],
...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    try {
        ...
        $pdo->prepare("DELETE FROM newsletter_blocks WHERE campaign_id = ?")->execute([$campaignId]);
        $pdo->prepare("DELETE FROM newsletter_campaigns WHERE id = ?")->execute([$campaignId]);
...
if ($tablesExist) {
    try {
        $stmt = $pdo->query("SELECT COUNT(DISTINCT email) as count FROM marketing_consents WHERE status = 'active' AND consent_marketing = 1");
        $subscribersCount = $stmt->fetch(PDO::FETCH_ASSOC)['count'] ?? 0;
        
        $stmt = $pdo->query("SELECT COUNT(DISTINCT email) as count FROM marketing_consents WHERE status = 'active' AND consent_marketing = 1 AND subscribed_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)");
        ...
        $stmt = $pdo->query("SELECT AVG((opened_count / NULLIF(sent_count, 0)) * 100) as rate FROM newsletter_campaigns WHERE status = 'sent' AND sent_count > 0");
        $openRate = round($stmt->fetch(PDO::FETCH_ASSOC)['rate'] ?? 0, 1);
        
        $stmt = $pdo->query("SELECT AVG((clicked_count / NULLIF(sent_count, 0)) * 100) as rate FROM newsletter_campaigns WHERE status = 'sent' AND sent_count > 0");
    ...
    try {
        $stmt = $pdo->query("SELECT * FROM newsletter_campaigns ORDER BY CASE status WHEN 'sending' THEN 1 WHEN 'scheduled' THEN 2 WHEN 'draft' THEN 3 WHEN 'sent' THEN 4 ELSE 5 END, created_at DESC LIMIT 20");
...
.stat-card__icon { width: 48px; height: 48px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 24px; margin-bottom: 12px; }
...
.stat-card__value { font-size: 28px; font-weight: 700; color: #111827; margin: 0 0 4px 0; }
.stat-card__label { font-size: 13px; color: #6B7280; margin: 0; }
.stat-card__change { font-size: 12px; margin-top: 8px; color: #10B981; }
...
.table-wrapper th { background: #F9FAFB; padding: 12px 16px; text-align: left; font-weight: 600; font-size: 11px; text-transform: uppercase; color: #6B7280; border-bottom: 1px solid #E5E7EB; }
.table-wrapper td { padding: 12px 16px; border-bottom: 1px solid #F3F4F6; font-size: 14px; }
...
.action-icons a { color: #6B7280; text-decoration: none; font-size: 18px; transition: all 0.2s; display: inline-flex; align-items: center; justify-content: center; width: 34px; height: 34px; border-radius: 6px; }
.action-icons a:hover { color: #2B59A6; background: #EFF6FF; }
.empty-state { text-align: center; padding: 60px 20px; color: #6B7280; }
.empty-state i { font-size: 48px; margin-bottom: 16px; opacity: 0.3; }
.progress-bar { width: 100%; height: 4px; background: #E5E7EB; border-radius: 2px; overflow: hidden; margin-top: 4px; }
.progress-bar__fill { height: 100%; background: #10B981; }
</style>
...
<div class="admin-content">
    ...
    <div class="stats-grid">
        <div class="stat-card stat-card--subscribers">
            ...
            <p class="stat-card__change"><i class="bi bi-arrow-up"></i> +<?php echo $newSubscribers; ?> w ostatnim miesiącu</p>
            <?php endif; ?>
        </div>
        ...
        <div class="stat-card stat-card--opened">
            <div class="stat-card__icon"><i class="bi bi-envelope-open"></i></div>
        ...
        <div class="stat-card stat-card--clicked">
            <div class="stat-card__icon"><i class="bi bi-cursor"></i></div>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <?php
                        $statusLabels = ['draft' => 'Szkic', 'sent' => 'Wysłana'];
                    ...
                    <td style="font-size: 13px;">
                        <?php echo $campaign['sent_at'] ? date('d.m.Y H:i', strtotime($campaign['sent_at'])) : date('d.m.Y H:i', strtotime($campaign['created_at'])); ?>
                    </td>
                    ...
                    <td style="text-align: center;">
                        <?php echo $campaign['sent_count'] > 0 ? round(($campaign['opened_count'] / $campaign['sent_count']) * 100, 1) : 0; ?>%
                    </td>
                    <td style="text-align: center;">
                        <div class="action-icons">
                            <?php if ($campaign['status'] === 'draft'): ?>
                            <a href="/admin/newsletter-edit.php?id=<?php echo $campaign['id']; ?>" title="Edytuj"><i class="bi bi-pencil"></i></a>
                            <a href="/admin/newsletter-send.php?id=<?php echo $campaign['id']; ?>" title="Wyślij"><i class="bi bi-send-fill"></i></a>
                            <a href="?duplicate=<?php echo $campaign['id']; ?>" title="Duplikuj"><i class="bi bi-files"></i></a>
                            <a href="?delete=<?php echo $campaign['id']; ?>" onclick="return confirm('Czy na pewno usunąć?')" title="Usuń"><i class="bi bi-trash"></i></a>
                            <?php else: ?>
                            <a href="/admin/newsletter-stats.php?id=<?php echo $campaign['id']; ?>" title="Statystyki"><i class="bi bi-bar-chart"></i></a>
                            <a href="?duplicate=<?php echo $campaign['id']; ?>" title="Duplikuj"><i class="bi bi-files"></i></a>
                            <a href="?delete=<?php echo $campaign['id']; ?>" onclick="return confirm('Czy na pewno usunąć?')" title="Usuń"><i class="bi bi-trash"></i></a>
                            <?php endif; ?>
                        </div>
