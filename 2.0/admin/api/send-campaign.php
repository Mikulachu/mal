<?php
/**
 * ADMIN/API/SEND-CAMPAIGN.PHP - Z DEBUGOWANIEM PRZEZ ECHO
 */
...
try {
    ...
    foreach ($subscribers as $email) {
        echo "Wysyłam do: $email ... ";
        
        try {
            $personalizedHTML = personalizeEmail($emailHTML, $email);
...
// FUNKCJE (takie same jak wcześniej)
    ...
    switch ($type) {
        ...
        case 'buttons-row':
            ...
            foreach ($buttons as $btn) {
                $html .= "<a href='{$btn['link']}' style='flex: 1; min-width: 140px; background: {$btn['bg_color']}; color: white; padding: 14px 20px; text-align: center; text-decoration: none; border-radius: 6px; font-weight: 600;'>" . htmlspecialchars($btn['text']) . "</a>";
            }
            $html .= "</div>";
            return $html;
        ...
        case 'social':
            ...
            return "<div style='text-align: center; margin: 24px 0;'>" . implode('', $links) . "</div>";
...
function personalizeEmail($html, $email) {
